...
One scheduled job ties together the prepared unit, job script, environment, and
generated artifacts:

```{mermaid}
%%{init: {"theme": "dark"}}%%
flowchart LR
  A["prepared unit<br/>notebook, script, or input"] --> C["Slurm job"]
  B["job script<br/>resources + command"] --> C
  D["cluster_env.sh<br/>software setup"] --> C
  C --> E["compute node"]
  E --> F["stdout log"]
  E --> G["stderr log"]
  E --> H["output artifact"]
